...
# TODO: TRANSFER ALL THE PRINT LINES INTO A LOG FILE, class Lg(info/warning, data/str).
...
def rate_limit_check(max_pc_packets, max_server_packets):
    ...
    print("TOTAL REQUESTS TO THE COMPUTER:", sum_all_requests(max_pc_packets))
    print("TOTAL REQUESTS TO THE SERVER:", sum_server_requests(max_server_packets))
...
def sum_all_requests(max_pc_packets):
    ...
    for user in pc_users:
        ...
        if packets > int(max_pc_packets) / 10:  # if the user sends over 2400 packets to the
...
def sum_server_requests(max_server_packets):
    ...
    for user in server_users:
        ...
        if packets > int(max_server_packets) / 10:  # if the user sends over 1000 packets to the
            # server in under 30 seconds he get blocked
            if not db.find_in_blacklist(ip):
                db.insert_to_blacklist(ip, datetime.now())
...
def main(port, server_ip, max_pc_packets, max_server_packets):
    ...
    :return: nothing
    """
    print(port, server_ip, max_pc_packets, max_server_packets)
